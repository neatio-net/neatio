...
func (n *cachedNode) childs() []common.Hash {
	...
	for side := range n.children {
		children = append(children, side)
	}
...
func (db *Database) reference(side common.Hash, parent common.Hash) {
	...
	} else if _, ok = db.dirties[parent].children[side]; ok && parent != (common.Hash{}) {
		return
	}
	node.parents++
	db.dirties[parent].children[side]++
}
...
func (db *Database) dereference(side common.Hash, parent common.Hash) {
	...
	if node.children != nil && node.children[side] > 0 {
		node.children[side]--
		if node.children[side] == 0 {
			delete(node.children, side)
		}
	}
...
func (db *Database) verifyIntegrity() {
	...
	for side := range db.dirties[common.Hash{}].children {
