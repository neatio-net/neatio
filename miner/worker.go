...
func (self *worker) start() {
	...
	if neatpos, ok := self.engine.(consensus.NeatPoS); ok {
		err := neatpos.Start(self.chain, self.chain.CurrentBlock, self.chain.HasBadBlock)
		if err != nil {
			self.logger.Error("Starting NeatPoS failed", "err", err)
		}
	}
...
func (self *worker) commitNewWork() {
	...
	if now := time.Now().Unix(); tstamp > now+1 {
		...
		//In neatio, there is no need to sleep to wait, commit work immediately
		//time.Sleep(wait)
	}
