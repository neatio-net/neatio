...
func (cch *CrossChainHelper) CanCreateSideChain(from common.Address, chainId string, minValidators uint16, minDepositAmount, startupCost *big.Int, startBlock, endBlock *big.Int) error {
	...
	if minDepositAmount.Cmp(officialMinimumDeposit) == -1 {
		return fmt.Errorf("Deposit amount is not meet the minimum official deposit amount (%v NIO)", new(big.Int).Div(officialMinimumDeposit, big.NewInt(params.Nio)))
	}

	if startupCost.Cmp(officialMinimumDeposit) != 0 {
		return fmt.Errorf("Startup cost is not meet the required amount (%v NIO)", new(big.Int).Div(officialMinimumDeposit, big.NewInt(params.Nio)))
	}
