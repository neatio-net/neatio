...
func (bc *BlockChain) ValidateBlock(block *types.Block) (*state.StateDB, types.Receipts, *types.PendingOps, error) {
	...
	// Header verify
	if err := bc.engine.(consensus.NeatPoS).VerifyHeaderBeforeConsensus(bc, block.Header(), true); err != nil {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	tdm := bc.Engine().(consensus.NeatPoS)
	...
	var reorg bool
	if _, ok := bc.engine.(consensus.NeatPoS); ok {
		// NeatPoS Engine always Canon State, insert the block to the chain,
		reorg = true
	} else {
