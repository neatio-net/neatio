...
import (
	...
	consss "github.com/neatlab/neatio/consensus"
...
type RoundState struct {
	...
	// Following fields are used for NeatPoS/BLS signature aggregation
...
func (cs *ConsensusState) createProposalBlock() (*types.TdmBlock, *types.PartSet) {
	...
	//here we wait for neatio block to propose
	if cs.blockFromMiner != nil {
...
// In NeatPoS, wait for 2/3 votes for prevote
...
// In NeatPoS, when prevote round ends, enter to vote for precommit
...
// In NeatPoS, wait for 2/3 votes for precommit
...
func (cs *ConsensusState) defaultSetProposal(proposal *types.Proposal) error {
	...
	if proposal.Round == cs.Round {
		...
		if !cs.GetProposer().PubKey.VerifyBytes(types.SignBytes(cs.chainConfig.NeatChainId, proposal), proposal.Signature) {
			...
			fmt.Printf("default set proposal1, cs.chainConfig.NeatChainId %v\n", cs.chainConfig.NeatChainId)
			fmt.Printf("default set proposal1, verifyBytes(cs, proposal.Signature) %v, %v\n", types.SignBytes(cs.chainConfig.NeatChainId, proposal), proposal.Signature.String())
		...
		if !cs.proposerByRound(proposal.Round).Proposer.PubKey.VerifyBytes(types.SignBytes(cs.chainConfig.NeatChainId, proposal), proposal.Signature) {
			...
			fmt.Printf("default set proposal2, cs.chainConfig.NeatChainId %v\n", cs.chainConfig.NeatChainId)
...
func (cs *ConsensusState) saveBlockToMainChain(block *ethTypes.Block) {
	...
	bs := []byte{}
	proofData, err := ethTypes.NewSideChainProofData(block)
	if err != nil {
