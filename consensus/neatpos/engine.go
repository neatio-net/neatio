...
var (
	...
	sideChainRewardAddress = common.StringToAddress("NEAT3HUowneGAMMUwnj8SfiATomdZdqv")
)
...
func (sb *backend) Start(chain consensus.ChainReader, currentBlock func() *types.Block, hasBadBlock func(hash common.Hash) bool) error {
	...
	sb.logger.Info("NeatPoS backend Start")
	...
	sb.chain = chain
	fmt.Printf("NeatPoS backend start chain %v\n", chain)
...
func (sb *backend) Stop() error {
	...
	sb.logger.Info("NeatPoS backend stop")
	...
	if !sb.core.Stop() {
		return errors.New("NeatPoS stop error")
	}
...
func (sb *backend) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	...
	sb.logger.Info("NeatPoS backend verify header")
...
func (sb *backend) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if header.Time.Cmp(big.NewInt(now().Unix())) > 0 {
		...
		//in neatio, avoid the time difference to tolerate time gap between nodes
	...
	if header.Number.Uint64() > sb.GetEpoch().EndBlock {
		for {
			...
			sb.logger.Infof("NeatPoS VerifyHeader, Epoch Switch, wait for %v then try again", duration)
			time.Sleep(duration)
	...
	if !sb.chainConfig.IsMainChain() {
		...
		for {
			...
			if tried == 10 {
				sb.logger.Warnf("NeatPoS VerifyHeader, Main Chain Number mismatch, after retried %d times", tried)
			...
			tried++
			sb.logger.Infof("NeatPoS VerifyHeader, Main Chain Number mismatch, wait for %v then try again (count %d)", duration, tried)
			time.Sleep(duration)
		}
	}
...
func (sb *backend) VerifyHeaderBeforeConsensus(chain consensus.ChainReader, header *types.Header, seal bool) error {
	sb.logger.Info("NeatPoS backend verify header before consensus")
...
func (sb *backend) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	...
	sb.logger.Info("NeatPoS backend verify headers")
...
func (sb *backend) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
	...
	sb.logger.Debugf("NeatPoS Finalize, receipts are: %v", receipts)
...
func (sb *backend) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (interface{}, error) {
	...
	sb.logger.Info("NeatPoS backend seal")
	...
	// post block into NeatPoS engine
	sb.logger.Infof("NeatPoS Seal, before fire event with block height: %d", block.NumberU64())
	...
	for {
		...
		case result, ok := <-sb.commitCh:
			...
			if ok {
				sb.logger.Debugf("NeatPoS Seal, got result with block.Hash: %x, result.Hash: %x", block.Hash(), result.Hash())
				// if the block hash and the hash from channel are the same,
				// return the result. Otherwise, keep waiting the next hash.
				if block.Hash() == result.Hash() {
					return result, nil
				}
				sb.logger.Debug("NeatPoS Seal, hash are different")
			} else {
				sb.logger.Debug("NeatPoS Seal, has been restart, just return")
				return nil, nil
			}
		...
		case iresult, ok := <-sb.vcommitCh:
			...
			if ok {
				sb.logger.Debugf("NeatPoS Seal, v got result with block.Hash: %x, result.Hash: %x", block.Hash(), iresult.Block.Hash())
				if block.Hash() != iresult.Block.Hash() {
					return iresult, nil
				}
				sb.logger.Debug("NeatPoS Seal, v hash are the same")
			} else {
				sb.logger.Debug("NeatPoS Seal, v has been restart, just return")
				return nil, nil
			}

		case <-stop:
			sb.logger.Debug("NeatPoS Seal, stop")
			return nil, nil
		}
	}
...
func (sb *backend) Commit(proposal *ncTypes.TdmBlock, seals [][]byte, isProposer func() bool) error {
	...
	sb.logger.Debugf("NeatPoS Commit, hash: %x, number: %v", block.Hash(), block.Number().Int64())
	sb.logger.Debugf("NeatPoS Commit, block: %s", block.String())

	if isProposer() && (sb.proposedBlockHash == block.Hash()) {
		sb.logger.Debugf("NeatPoS Commit, proposer | feed to Seal: %x", block.Hash())
		sb.commitCh <- block
		return nil
	} else {
		if proposal.IntermediateResult != nil {
			sb.logger.Debugf("NeatPoS Commit, validator | feed to Seal: %x", block.Hash())
			proposal.IntermediateResult.Block = block
			sb.vcommitCh <- proposal.IntermediateResult
		} else {
			sb.logger.Debugf("NeatPoS Commit, validator | fetcher enqueue: %x", block.Hash())
